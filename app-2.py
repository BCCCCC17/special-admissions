...
def load_data():
    ...
    # 🛡️ 防護機制：自動現場補齊任何可能缺失的解析欄位 (Self-Healing)
...
if kw_exp:
    filtered_df = filtered_df[filtered_df['系所招生對象'].astype(str).str.contains('實驗教育|自學', na=False)]
if kw_cs:
    filtered_df = filtered_df[filtered_df['Search_Tags'].astype(str).str.contains('CS|Computer Science', na=False)]
if kw_lang:
    filtered_df = filtered_df[filtered_df['Search_Tags'].astype(str).str.contains('English|Languages|Bilingual', na=False)]
...
if search_kw.strip():
    ...
    filtered_df = filtered_df[
        ...
        filtered_df['Search_Tags'].astype(str).str.contains(kw, na=False) |
